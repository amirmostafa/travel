...
export class LoyaltyProgramService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/loyalty-programs');
  ...
  getLoyaltyProgramIdentifier(loyaltyProgram: Pick<ILoyaltyProgram, 'id'>): number {
  ...
  addLoyaltyProgramToCollectionIfMissing<Type extends Pick<ILoyaltyProgram, 'id'>>(
    ...
    if (loyaltyPrograms.length > 0) {
      const loyaltyProgramCollectionIdentifiers = loyaltyProgramCollection.map(loyaltyProgramItem =>
        this.getLoyaltyProgramIdentifier(loyaltyProgramItem),
      );

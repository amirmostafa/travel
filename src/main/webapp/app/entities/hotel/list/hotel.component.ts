...
import { EntityArrayResponseType, HotelService } from '../service/hotel.service';
...
export class HotelComponent implements OnInit {
  ...
  trackId = (_index: number, item: IHotel): number => this.hotelService.getHotelIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.hotels = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IHotel[] | null): IHotel[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const hotelsNew = this.hotels ?? [];
      if (data) {
        for (const d of data) {
          if (hotelsNew.map(op => op.id).indexOf(d.id) === -1) {
            hotelsNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.hotelService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
